...
| <a name="input_hypervisor_control_ip_address"></a> [hypervisor\_control\_ip\_address](#input\_hypervisor\_control\_ip\_address) | (IWE ONLY) The hypervisor control virtual IP address for the HyperFlex compute cluster that is used for node/pod management. | `string` | `""` | no |
...
| <a name="input_cluster_internal_subnet"></a> [cluster\_internal\_subnet](#input\_cluster\_internal\_subnet) | (IWE ONLY) A CIDR subnet for the cluster internal network. | <pre>object({<br>    gateway  = string<br>    ip_address  = string<br>    netmask  = string<br>  })</pre> | n/a | no |
...
| <a name="input_storage_client_vlan"></a> [storage\_client\_vlan](#input\_storage\_client\_vlan) | (IWE ONLY) The VLAN for the HyperFlex storage client traffic. | <pre>object({<br>    name  = string<br>    vlan_id  = number<br>    ip_address  = string<br>    netmask  = string<br>  })</pre> | n/a | no |
